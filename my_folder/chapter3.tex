...
В данной главе представлена разработка тестовой документации для приложения топологической сортировки, реализующего алгоритмы DFS и Кана. Сначала определены проверяемые условия (test cover items), затем для каждого метода тестирования разработаны контрольные примеры (test cases).
В завершение главы приведён сравнительный анализ использованных методов с оценкой трудозатрат и рекомендациями по оптимизации тестового набора ~\cite{iso29119-1, iso29119-2}.

\section{Проверяемые условия (Test Cover Items)} \label{ch3:items}

Проверяемые условия определяют, какие аспекты системы подлежат тестированию. Они разделены на три категории: входные данные, выходные данные и внутренняя обработка.

\subsection{Входные данные} \label{ch3:items:input}
...
\subsection{Выходные данные} \label{ch3:items:output}
...
\subsection{Внутренняя обработка} \label{ch3:items:proc}
...
\subsection{Ветви алгоритма DFS}
...
\subsection{Сводная таблица характеристик}
...
\subsection{Детальное сравнение методов}

В таблице~\ref{tab:comparison_detailed} представлено детальное сравнение методов по различным критериям.
...
В таблице~\ref{tab:effort} представлена оценка трудозатрат на применение каждого метода тестирования.
...
Общие трудозатраты на разработку тестовой документации составили 17 человеко-часов. Наиболее трудоёмким оказался метод Branch Testing (6.5 ч), наименее трудоёмким --- Equivalence Partitioning (2.5 ч).

Рекомендуется применять комбинацию методов EP и BVA для базового покрытия входных данных, дополняя их Decision Table Testing для проверки бизнес-логики и Branch Testing для обеспечения структурного покрытия кода.