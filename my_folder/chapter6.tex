...
В параграфе~\ref{ch6:architecture} описана архитектура приложения.
Параграф~\ref{ch6:model} посвящён модели данных.
В параграфе~\ref{ch6:algorithms} рассматриваются алгоритмы сортировки. Параграф~\ref{ch6:io} описывает подсистему ввода-вывода. В параграфе~\ref{ch6:validation} представлены утилиты валидации. Параграф~\ref{ch6:app} содержит описание консольного приложения.
...
Класс \texttt{Graph} представляет ориентированный граф с использованием списка смежности. Выбор данной структуры данных обусловлен эффективностью операций обхода, критичных для алгоритмов топологической сортировки.
...
\subsection{DFS-алгоритм}\cite{bhargava-algorithms-ru}


Класс \texttt{DfsTopologicalSort} реализует топологическую сортировку на основе поиска в глубину (Depth-First Search). Алгоритм использует трёхцветную маркировку вершин для отслеживания состояния обработки.
...
\subsection{Алгоритм Кана}\cite{bhargava-algorithms-ru}


Класс \texttt{KahnTopologicalSort} реализует алгоритм Кана, основанный на последовательном удалении вершин с нулевой входящей степенью.
...
В данной главе представлено краткое описание реализации программы топологической сортировки графов. Основные результаты:

\begin{enumerate}
    \item Разработана модульная архитектура с чётким разделением ответственности между компонентами.

    \item Реализована модель данных с использованием паттернов <<неизменяемый объект>> и <<строитель>>.
...
Общий объём исходного кода составляет около 700 строк (без учёта комментариев и пустых строк). Код соответствует принципам SOLID и обеспечивает возможность расширения системы новыми алгоритмами.
